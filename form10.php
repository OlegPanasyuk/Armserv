<html>
<head>
	<title>Срезы отопления часовые</title>
  ...
  $cnt1=0;  $cnt2=0;
   $result=mysql_query("select item_name FROM objects WHERE  item_parent_id=-1");
    ...
    if ($res=mysql_fetch_array($result,MYSQL_BOTH))
	{
	 ...
	 $name=$res[1];
	 if (($n_type==7) or ($n_type==9)) {if ($ctype==4 or $c_type=6) {$znum= "Номер счетчика ".$res[2];$zn=$res[2];} }
	 else {$znum=""; $zn="";}
	}
	if ($n_type == 7) {
...
//	 echo "			<td rowspan='2' height="34" class='x23' style='border-left:none;width:10px;'>Р’Р—</td>\n";
	
	 if ($pw_en==2) echo "			<td  colspan='".(count($nrg)*2)."' class='x23' style='border-left:none' align='center'>Отопление</td>\n";
...
function PrintSumData($pw_en,$date_1,$uid,$n_obj,$adr,$table)
{
 ...
 $colspan=count($nrg)*2;
	...
	if (count($nrg) == 0) {} else {
		if ($pw_en==2) echo "<td class='x23'  colspan=".$colspan."  align='center'>Суммарные показатели измениния измерений</td>\n";
	echo "</tr>\n";
 	$count=incrementProgressBar();
	$count=incrementProgressBar();
	 for ($i=0;$i<count($nrg);$i++)
       ...
       if ($pw_en==2)
	   {
	    ...
	    $count=incrementProgressBar();
		  ...
		  if ($pw_en==2)
		   {
			echo "<tr>\n";
			echo "	<td height='20' class='x28' align='center' style='border-right:none;'>За день</td>\n";
			for ($i=0;$i<count($nrg);$i++)
			{
...
//================================================================================================
	...
	{
	$isHeatSeason="Р›РµС‚Рѕ";$seas=2;} 
	else {
	$isHeatSeason="Р—РёРјР°";$seas=1;}
	}
//================================================================================================
 if ($pw_en==1) //СЃС‡РёС‚Р°РµРј РјРѕС‰РЅРѕСЃС‚СЊ
 {
  if ($node==-1)//РµСЃР»Рё РѕР±РѕРіСЂРµРІ
  ...
  else if ($node==-2)//РµСЃР»Рё РіРµРЅРµСЂР°С‚РѕСЂ
  {
 ...
 else //СЃС‡РёС‚Р°РµРј СЌРЅРµСЂРіРёСЋ
  {
  if ($node==-1)//РµСЃР»Рё РѕР±РѕРіСЂРµРІ
   $result3=mysql_query("select n_zone,color,descript,timezone_h FROM zones where n_zone in (select distinct heat_zone from intervals) order by n_zone");
  else if ($node==-2)//РµСЃР»Рё РіРµРЅРµСЂР°С‚РѕСЂ
 ...
 {
	$i_type=$nrg[$i];
	if ($node==-1 and $n_zone!=2)//РѕР±РѕРіСЂРµРІ
	{
	 ...
	 else if ($node==-2)//РіРµРЅРµСЂРёСЂСѓС‰РёР№
	 {
		if ($seas==1)//СЃРµР·РѕРЅ Р·РёРјР°
			{
			 if ($i_type==2)
			 { 
		...
		else if ($seas==2)//СЃРµР·РѕРЅ Р»РµС‚Рѕ
			{
...
//=====================================================================================================================	
 ...
 //========================РѕС‚СЂРёСЃРѕРІРєР° Р·РѕРЅ==================================================
	 if ($pw_en==1)
	 {
      echo "<tr bgcolor=".$bgcolor.">\n";
		echo "<td height='15' rowspan='1' class='x23' style='border-bottom:none;border-right:none;width:150px;'>С‚Р°СЂРёС„ ".$tariff_name."</td>\n";
  		//РІС‹РІРѕРґ РјРѕС‰РЅРѕСЃС‚Рё 
		for ($i=0;$i<count($nrg);$i++)
		{
	 ...
	 echo "</tr>\n";	
	   ...
	   {	
     echo "<tr bgcolor=".$bgcolor.">\n";
		echo "<td height='17' class='x23' style='border-right:none;width:150px;'>С‚Р°СЂРёС„ ".$tariff_name."</td>\n";
	  //РІС‹РІРѕРґ СЌРЅРµСЂРіРёРё 
		for ($i=0;$i<count($nrg);$i++)
		{
