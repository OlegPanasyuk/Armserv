...
echo "<script language=\"JavaScript1.2\" src=\"js/control_sv2.js\"></script>\n";
...
if (!isset($pw_en)) $pw_en=$disp;
 ...
 $type_izm = array(); //типы измерений забираемых со счетчика для отображения
 $pwr = array();// типы для мощности
 $nrg = array();//типы для энергии и показаний
 $colname = array();// имена измерений
...
//============================================================================================================
function setInterval()// устанавливаем интервал дат
{
global $date_1;
global $date_2;
	$count=incrementProgressBar();
...
function PrintHeader($pw_en,$uid,$n_obj,$adr,$date_2) //отображение заголовка таблицы
{
  global $type_izm;
  global $pwr;
  global $nrg;
...
//=====================определяем параметры и название точки учета===================================
   $result=mysql_query("select item_name FROM objects WHERE  item_parent_id=-1");
    ...
    if ($res=mysql_fetch_array($result,MYSQL_BOTH))
	{
	 $izm_str=$res[0];	$type_izm = explode (",",$izm_str); $ktr=$res[4];$ctype=$res[3]; echo $ktr;
	 $name=$res[1]; 
 ...
 for ($i=0;$i<count($type_izm);$i++)
 {
  ...
  /*or ($i=0;$i<count($nrg);$i++)
	...
	$col = count($nrg)*2+1 + 4;
	 ...
	 echo "	<td  colspan='".(count($nrg)*2 + 4)."' class='x23' style='border-left:none' align='center'>Энергия</td>\n";
	 echo "		</tr>\n";
	 echo "		<tr>\n";
  ...
  $pwr_str='';$nrg_str='';
	...
	$count=incrementProgressBar();
	 ...
	 {	  $m1=min($nrg);$m2=max($nrg);	 }
	    ...
	    for ($i=4;$i<count($nrg);$i++)
		  {
		   $nrg_str.=$nrg[$i].',';
		  }
		$nrg_str="5,6,7,8";
	  	$result2=mysql_query("select znach,flag,data,izm_type FROM ".$table." WHERE ".$table.".data BETWEEN '".$date_1."' AND '".$date_2."'  AND ".$table.".n_obj=".$n_obj." AND link_adr=".$adr."  AND ".$table.".izm_type IN(".$nrg_str.")  and ".$table.".inter_val=48 GROUP BY data,izm_type");
 ...
 echo "<td class='x22' align='center' style='font-weight:700;text-align:center;border-top:none;' id='".$date_output."'>".$date_output."</td>\n";
	...
	if ($n<1) {
		...
		for ($j=4;$j<=7;$j++) {
			...
			echo "	<td class='x22' style='border-top:none; border-left:none;'>&nbsp;".format((($izm[$n][$j]-$izm[$n-1][$j])*$ktr-$izm[$n][$j-4])/$edizm)."</td>\n";
		}
	}
	 echo "</tr>\n";	
  }	 
