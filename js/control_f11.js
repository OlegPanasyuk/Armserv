...
control+="<FIELDSET ID='fld3' style='width:100%;background-color:#c1cdcd;border:2px solif #fff'>";
...
control+="  <select name='disp' size='1' style='width:120px;'>";
control+="	<option value='2'SELECTED>объем за часы</option>";
control+="	<option value='3'>показания</option>";
control+="   </select>";
control+="</TD>";
control+="  <TD style='width:130px;'>отображать контур</TD>";
control+="  <TD style='width:90px;'>";
control+="   <select name='disp_circle' size='1' style='width:90px;'>";
...
control+="<TD style='width:60px;'>за день&nbsp;</TD>";
control+="<td style='width:110px;'><input name='dc' value='' size='9' readonly style='font-weight:bolder'>";
...
control+="<td><input type='submit' value='чтение' onclick='checkID()' style=\"font-size:12px;font-weight:bolder;\">     ";
...
control+="<input type='hidden' name='frameName' id='frameName' value='form10'>";
control+="  </TD>";
...
//CloseXls();
 ...
 {
  alert("ќшибка экспорта: "+e.name);return 0;
 }
}
...
if(ydiff!=0){ movey=Math.round(ydiff/1); ctrl_height_start-=movey}
if(xdiff!=0) { movex=Math.round(xdiff/1); ctrl_width_start-=movex}
