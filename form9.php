...
echo "<script language=\"JavaScript1.2\" src=\"js/control_f9.js\"></script>\n";
...
if (isset($date_2) and isset($uid) and isset($n_obj) and isset($adr) and isset($pw_en) and ($node==2 or $node==3) )
{
...
function CalcData($pw_en,$date_1,$date_2,$uid,$n_obj,$adr,$table)
{
  ...
  $pwr_str='';$nrg_str='';
	...
	$count=incrementProgressBar();
	 ...
	 {	  $m1=min($nrg)-1;$m2=max($nrg)+1;	 }
	  ...
	  if ($pw_en==2)  
	  {
		...
		$result2=mysql_query("select znach, flag, data, izm_type FROM ".$table." WHERE ".$table.".data = DATE_ADD('".$date_1."',INTERVAL -1 day)  AND ".$table.".n_obj=".$n_obj." AND link_adr=".$adr." AND ".$table.".izm_type IN(".$nrg_str.") AND inter_val=48 GROUP BY data,izm_type");
	  }
	  if ($pw_en==3)  
	  {
	    for ($i=0;$i<count($nrg);$i++)
    ...
    $count=incrementProgressBar();
	$i=0;
	$result=mysql_query("select distinct data FROM ".$table." WHERE ".$table.".data BETWEEN '".$date_1."' AND '".$date_2."'  AND ".$table.".n_obj=".$n_obj." AND link_adr=".$adr." AND ".$table.".izm_type between ".($m1-13)." and ".($m2-13)." ");
     if ($result)
	 {
	...
	for ($j=(min($nrg)-1);$j<max($nrg);$j++)
	 {
	  ...
	  if (in_array(($j+1),$nrg))
	  {
	    if ($j>0) echo "	<td class='x22' style='border-top:none;border-left:none;'>&nbsp;".format($izm[$n][$j-12]/$edizm)."</td>\n";
		else echo "	<td class='x22' style='border-top:none; border-left:none;'>&nbsp;".format($izm[$n][$j-12]/$edizm)."</td>\n";
	    echo "	<td class='x22' style='border-top:none;border-left:none' align='right'>&nbsp;".$flag[$n][$j-12]."</td>\n";
	  }	
	 }	 
	if ($pw_en==3)
	for ($j=(min($nrg)-1);$j<max($nrg);$j++)
	 {
	  ...
	  if (in_array(($j+1),$nrg))
	  {
		if ($j>4) echo "	<td class='x22' style='border-top:none;border-left:none;'>&nbsp;".format($izm[$n][$j-13]/$edizm)."</td>\n";
		else echo "	<td class='x22' style='border-top:none;'>&nbsp;".format($izm[$n][$j-13]/$edizm)."</td>\n";
	    echo "	<td class='x22' style='border-top:none;border-left:none' align='right'>&nbsp;".$flag[$n][$j-13]."</td>\n";
	  }	
	 }	 
	 echo "</tr>\n";	
  }	 
  ...
  global $summ_arr;
	...
	$count=incrementProgressBar();		
	 for ($i=0;$i<count($colname);$i++)
	 {
		...
		{
			$izm_type -= 12; 
			$izm[$i]=$summ_arr[$izm_type - 1];$flag[$i]="\n";
...
//==========================================================================================================================
	  {
		...
		if ($pw_en==2) echo "<td class='x23'  colspan=".(count($colname)*2)."  align='center'>суммарный объем</td>\n";
...
else echo "<div id='activator' ONCLICK=ToXls_6('TB',2,'".$pw_en."','".$count."','".$num_day."','".$ac_day.".".$ac_month.".".$ac_year."','".$zn."',0)></div>\n"; 
?>
<center>
