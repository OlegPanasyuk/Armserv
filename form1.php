...
echo "<script language=\"JavaScript1.2\" src=\"js/control_f1.js\"></script>";
...
if (isset($date_1) and isset($uid) and isset($n_obj) and isset($adr) and isset($pw_en) and ($node<2 or $node==5) )
{
...
//			 echo $begin_date[$res4[0]-1]."_".$end_date[$res4[0]-1]."<br>";
 ...
 if (($result2) and (mysql_num_rows($result2)>0))
  {	
	 ...
	 //завершение получения нач показаний
     ...
     for ($i=0;$i<count($colname);$i++)
	 {
		...
		else		   
		{
		 if ($node==-2)
		 {
			 ...
			 if ($i_type==2) $result=mysql_query("select ".$table.".znach,".$table.".flag,".$table.".inter_val,beg_int,end_int ,intervals.winter_zone,intervals.max_zone,intervals.heat_zone,intervals.n_zone  
			 FROM ".$table." ,intervals WHERE ".$table.".data='".$date_1."' AND izm_type=".$i_type." AND n_obj=".$n_obj." AND link_adr=".$adr."  AND ".$table.".inter_val=".$int_val." And ".$table.".inter_val = intervals.inter_val Order By ".$table.".inter_val, ".$table.".izm_type");
	...
	$count=incrementProgressBar();
	 for ($i=0;$i<count($nrg);$i++)
	  ...
	  if ($pw_en==1)//считать мощность
	  {
	   ...
	   {
		  // если объект на генерации
		  if ($node==-2)
		    {
			// для Р-
			if ( $i_type==2)
			{
				$result=mysql_query(" SELECT s.mx,i.beg_int,i.end_int,s.inter_val,i.winter_zone FROM (SELECT    MAX(znach) as mx, inter_val FROM ".$table." WHERE  ".$table.".data='".$date_1."' AND ".$table.".n_obj=".$n_obj." AND link_adr=".$adr." AND ".$table.".izm_type=".$i_type." GROUP BY inter_val ORDER BY mx DESC) s, intervals i  
				WHERE s.inter_val = i.inter_val  and i.winter_zone in(3,4)");
			}	else {
						$result=mysql_query(" SELECT s.mx,i.beg_int,i.end_int,s.inter_val,i.max_zone FROM (SELECT    MAX(znach) as mx, inter_val FROM ".$table." WHERE  ".$table.".data='".$date_1."' AND ".$table.".n_obj=".$n_obj." AND link_adr=".$adr." AND ".$table.".izm_type=".$i_type." GROUP BY inter_val ORDER BY mx DESC) s, intervals i  
						WHERE s.inter_val = i.inter_val  and i.max_zone in(3,4)");
					}
			// конец для Р-			
