<html>
<head>
	<title>Разделение по сменам.</title>
...
echo "<script language=\"JavaScript1.2\" src=\"js/control_f10.js\"></script>";
...
PrintSmena ($pw_en,$date_1,$uid,$n_obj,$adr,$table);
...
//	 echo "			<td rowspan='2' height="34" class='x23' style='border-left:none;width:10px;'>ВЗ</td>\n";
	 ...
	 if ($pw_en==3) echo "			<td  colspan='".(count($nrg)*2)."' class='x23' style='border-left:none' align='center'>Показания</td>\n";
	 echo "		</tr>\n";
	$count=incrementProgressBar();
	 echo "		<tr>\n";
...
function PrintData($pw_en,$date_1,$uid,$n_obj,$adr,$table)
{
  ...
  $izm = array(); 	  $flag = array();$start_pokaz = array();
   if ($pw_en==3) $result2=mysql_query("select inter_val FROM ".$table." WHERE ".$table.".data='".$date_1."' AND izm_type=".($nrg[0]-4)." AND n_obj=".$n_obj." AND link_adr=".$adr." ");
   else 
   {
    $result2=mysql_query("select inter_val FROM ".$table." WHERE ".$table.".data='".$date_1."' AND izm_type=".$nrg[0]." AND n_obj=".$n_obj." AND link_adr=".$adr." ");
   	$result3=mysql_query("select n_zone,color FROM zones order by n_zone");
...
//			 echo $ac_day.".".$ac_month.".".$ac_year."_";
...
//			 echo $begin_date[$res4[0]-1]."_".$end_date[$res4[0]-1]."<br>";
 ...
 if (($result2) and (mysql_num_rows($result2)>0))
  {	
     if ($pw_en==3)
	 {//расчет показаний через срезы энергии
     for ($i=0;$i<count($colname);$i++)
	  {
		$i_type=$nrg[$i];
	  	$result=mysql_query("select ".$table.".znach,".$table.".flag,".$table.".inter_val,beg_int,end_int   FROM ".$table." ,intervals WHERE ".$table.".data=DATE_ADD('".$date_1."',interval -1 DAY) AND izm_type=".$i_type." AND n_obj=".$n_obj." AND link_adr=".$adr."  AND ".$table.".inter_val=48 And ".$table.".inter_val = intervals.inter_val Order By ".$table.".inter_val, ".$table.".izm_type");
		if ($res=mysql_fetch_array($result,MYSQL_BOTH))
		$start_pokaz[$i]=$res[0];
	  }	
	 }
	...
	while ($res2=mysql_fetch_array($result2,MYSQL_NUM))
	{
	 $j=$res2[0];
	 $int_val=$j;
	 if ($j%5==0) $count=incrementProgressBar();
     for ($i=0;$i<count($colname);$i++)
	 {
		$i_type=$nrg[$i];
		if ($pw_en==3) $result=mysql_query("select ".$table.".znach,".$table.".flag,".$table.".inter_val,beg_int,end_int ,intervals.n_zone,intervals.max_zone  FROM ".$table." ,intervals WHERE ".$table.".data='".$date_1."' AND izm_type=".($i_type-4)." AND n_obj=".$n_obj." AND link_adr=".$adr."  AND ".$table.".inter_val=".$int_val." And ".$table.".inter_val = intervals.inter_val Order By ".$table.".inter_val, ".$table.".izm_type");
		else		   
		{
		 if ($node==-2)
		 {
		  if ($seas==1)	 
		  	{
			 if ($i_type==2) $result=mysql_query("select ".$table.".znach,".$table.".flag,".$table.".inter_val,beg_int,end_int ,intervals.winter_zone,intervals.max_zone,intervals.heat_zone  FROM ".$table." ,intervals WHERE ".$table.".data='".$date_1."' AND izm_type=".$i_type." AND n_obj=".$n_obj." AND link_adr=".$adr."  AND ".$table.".inter_val=".$int_val." And ".$table.".inter_val = intervals.inter_val Order By ".$table.".inter_val, ".$table.".izm_type");
			 else $result=mysql_query("select ".$table.".znach,".$table.".flag,".$table.".inter_val,beg_int,end_int ,intervals.n_zone,intervals.max_zone,intervals.heat_zone  FROM ".$table." ,intervals WHERE ".$table.".data='".$date_1."' AND izm_type=".$i_type." AND n_obj=".$n_obj." AND link_adr=".$adr."  AND ".$table.".inter_val=".$int_val." And ".$table.".inter_val = intervals.inter_val Order By ".$table.".inter_val, ".$table.".izm_type");
			} 
		  else if ($seas==2)
		  	{
			 if ($i_type==2) $result=mysql_query("select ".$table.".znach,".$table.".flag,".$table.".inter_val,beg_int,end_int ,intervals.summer_zone,intervals.max_zone,intervals.heat_zone  FROM ".$table." ,intervals WHERE ".$table.".data='".$date_1."' AND izm_type=".$i_type." AND n_obj=".$n_obj." AND link_adr=".$adr."  AND ".$table.".inter_val=".$int_val." And ".$table.".inter_val = intervals.inter_val Order By ".$table.".inter_val, ".$table.".izm_type");
			 else $result=mysql_query("select ".$table.".znach,".$table.".flag,".$table.".inter_val,beg_int,end_int ,intervals.n_zone,intervals.max_zone,intervals.heat_zone  FROM ".$table." ,intervals WHERE ".$table.".data='".$date_1."' AND izm_type=".$i_type." AND n_obj=".$n_obj." AND link_adr=".$adr."  AND ".$table.".inter_val=".$int_val." And ".$table.".inter_val = intervals.inter_val Order By ".$table.".inter_val, ".$table.".izm_type");
			} 
		 } 
		 else		 
	    ...
	    if ($res=mysql_fetch_array($result,MYSQL_BOTH))
		{
		 ...
		 if ($pw_en==3) {$start_pokaz[$i]+=$res[0]/$ktr;$izm[$i][$j]=$start_pokaz[$i];} 
		 else $izm[$i][$j]=$res[0];
		 $flag[$i][$j]=$res[1];  $int_val=$res[2];    
	     $begin = $res[3]; 	$end = $res[4]; 
	 ...
	 echo "<tr>\n";
	 echo "	<td  class='x26' align='center'  style='border-top:none;	padding-right: 1px;' >".$begin.'-'.$end."</td>\n";
...
function PrintSumData($pw_en,$date_1,$uid,$n_obj,$adr,$table)
{
 ...
 function PrintSmena ($pw_en,$date_1,$uid,$n_obj,$adr,$table)
 {
	global $type_izm;
  global $pwr;
  global $nrg;
  ...
  global $colname;
   ...
   global $edizm;
	...
	{	
	 ...
	 if ($pw_en==2)
	   {
		$result2=mysql_query("select SUM(znach),MAX(flag) FROM ".$table." WHERE ".$table.".data='".$date_1."'  AND n_obj=".$n_obj." AND link_adr=".$adr."  AND inter_val<=14 AND inter_val>=1 AND izm_type=".$i_type." group by izm_type order by izm_type");
	    if ($res2=mysql_fetch_array($result2,MYSQL_NUM)) 
		 {
			if ($pw_en<>1) $izm[$i]=$res2[0];
			$flag[$i]=$res2[1];
	 ...
	 if ($pw_en==2)  
		  ...
		  //вывод энергии 
			echo "<tr>\n";
			echo "	<td height='20' class='x26' align='center' style='border-right:none;'>0.00-7.00</td>\n";
			for ($i=0;$i<count($nrg);$i++)
			{
		    if ($i>0) echo "	<td class='x22' style='border-top:none;border-left:none;'>&nbsp;".format($izm[$i]/$edizm)."</td>\n";
			else echo "	<td class='x22' style='border-top:none;'>&nbsp;".format($izm[$i]/$edizm)."</td>\n";
	...
	{	
	 ...
	 if ($pw_en==2)
	   {
		$result2=mysql_query("select SUM(znach),MAX(flag) FROM ".$table." WHERE ".$table.".data='".$date_1."'  AND n_obj=".$n_obj." AND link_adr=".$adr."  AND inter_val<=29 AND inter_val>=15 AND izm_type=".$i_type." group by izm_type order by izm_type");
	    if ($res2=mysql_fetch_array($result2,MYSQL_NUM)) 
		 {
			if ($pw_en<>1) $izm[$i]=$res2[0];
			$flag[$i]=$res2[1];
	 ...
	 if ($pw_en==2)  
		  ...
		  //вывод энергии 
			echo "<tr>\n";
			echo "	<td height='20' class='x26' align='center' style='border-right:none;'>7.00-14.00</td>\n";
			for ($i=0;$i<count($nrg);$i++)
			{
		    if ($i>0) echo "	<td class='x22' style='border-top:none;border-left:none;'>&nbsp;".format($izm[$i]/$edizm)."</td>\n";
			else echo "	<td class='x22' style='border-top:none;'>&nbsp;".format($izm[$i]/$edizm)."</td>\n";
	...
	{	
	 ...
	 if ($pw_en==2)
	   {
		$result2=mysql_query("select SUM(znach),MAX(flag) FROM ".$table." WHERE ".$table.".data='".$date_1."'  AND n_obj=".$n_obj." AND link_adr=".$adr."  AND inter_val<=48 AND inter_val>=30 AND izm_type=".$i_type." group by izm_type order by izm_type");
	    if ($res2=mysql_fetch_array($result2,MYSQL_NUM)) 
		 {
			if ($pw_en<>1) $izm[$i]=$res2[0];
			$flag[$i]=$res2[1];
